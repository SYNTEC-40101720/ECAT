...
- TxPDO：`0x1C13 = 0x1A00`，本次实机读回 12 bytes，映射为 `0x6064`（32 bit）、
	`0x6041`（16 bit）、`0x60B9`（16 bit）、`0x60BA`（32 bit）；ESI 中的 19 bytes
	完整反馈布局仍作为兼容候选，Runtime 会拒绝未确认的其它映射
- 配置方式：`config_overlap_map()` 可进入 SAFE-OP
- 预期 WKC：`3`
- Profile Velocity：`0x6060 = 0x03`；12 bytes 布局不含 `0x6061`，Runtime 按当前命令模式
	作为模式显示值
- 故障复位：写 `0x6040 = 0x0080` 已验证
- 2026-09-04 故障排查：驱动启动到 OP 后报告 `statusword=0x0638`，只读 SDO 为
	`0x1001=0x10`（通信错误类别）及 `0x603F=0x821B`；从站 SAFE-OP、AL 状态和 AL
	错误码均为 `0x0000`，PDO 长度和预期 WKC=`3` 正常。当前 12 bytes TxPDO 不含
